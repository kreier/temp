...
Data and details can be found at:

- https://io.adafruit.com/kreier/dashboards/temperature
- https://io.adafruit.com/kreier/feeds/temperature
- https://io.adafruit.com/kreier/feeds/lipo

And for the solar project:
...
![Video with TFT MQTT cycle 12 seconds](2022-04-11_mqtt_tft.mov)
...
The fourth line on the TFT display reads the free memory from the garbage collector `gc.mem_free()`. It is constantly decreasing until no more submission over WIFI is done, but no error message is given. We publish this to MQTT as well and see the declining memory:

![Decreasing free memory](2022-04-10_memory_leak.jpg)

The workaround: Measure the free memory, and collect the trash using `gc.collect()` one the value is below a certain threshold. In case of our Metro ESP32-S2 setup, we just collect every 20 seconds.
